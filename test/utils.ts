...
export async function deployContractsFixture() {
  ...
  const ProtocolFactory = await ethers.getContractFactory("ZooProtocol");
  expect(ProtocolFactory.bytecode.length / 2).lessThan(maxContractSize);
  const Protocol = await ProtocolFactory.deploy();
  const protocol = ZooProtocol__factory.connect(await Protocol.getAddress(), provider);
  ...
  const InfraredBribeVaultFactory = await ethers.getContractFactory("InfraredBribeVaultV2", {
    libraries: {
