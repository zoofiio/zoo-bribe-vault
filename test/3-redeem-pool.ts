...
describe('RedeemPool', () => {
  ...
  it('RedeemPool works with iBGT', async () => {
    ...
    await expect(redeemPool.connect(Alice).redeem(ethers.parseUnits("0.1"), {value: ethers.parseUnits("0.001"),})).to.be.rejectedWith('msg.value should be 0');
    await expect(redeemPool.connect(Alice).redeem(ethers.parseUnits("0.1"), {value: ethers.parseUnits("1"),})).to.be.rejectedWith('msg.value should be 0');
    ...
    expect(await redeemPool.earnedAssetAmount(Alice.address)).to.equal(ethers.parseUnits("0.001", await iBGT.decimals()));
    expect(await redeemPool.earnedAssetAmount(Bob.address)).to.equal(ethers.parseUnits("0.002", await iBGT.decimals()));
    trans = await redeemPool.connect(Alice).claimAssetToken();
    await expect(trans).to.changeTokenBalances(
      iBGT,
      [Alice.address, await redeemPool.getAddress()],
      [ethers.parseUnits("0.001", await iBGT.decimals()), -ethers.parseUnits("0.001", await iBGT.decimals())]
    );
    await expect(trans).to.emit(redeemPool, "AssetTokenClaimed").withArgs(Alice.address, ethers.parseUnits("0.001", await iBGT.decimals()));
    ...
    await expect(trans).to.changeTokenBalances(
      iBGT,
      [Bob.address, await redeemPool.getAddress()],
      [ethers.parseUnits("0.002", await iBGT.decimals()), -ethers.parseUnits("0.002", await iBGT.decimals())]
    );
    await expect(trans).to.emit(redeemPool, "AssetTokenClaimed").withArgs(Bob.address, ethers.parseUnits("0.002", await iBGT.decimals()));
